  ...
  return (
    <Page3 title="Customers" sub="CRM, loyalty, gift cards and segments" actions={
      <>
        ...
        <button className="btn btn-primary"><Icon.Plus size={14}/>Add Customer</button>
      </>
    }>
      ...
      <div className="row-between" style={{marginBottom:12, flexWrap:'wrap', gap:8}}>
        ...
        <div style={{display:'flex', gap:8}}>
          <select className="btn btn-ghost"><option>All segments</option></select>
          <div style={{position:'relative'}}>
            <Icon.Search size={14} style={{position:'absolute', left:10, top:11, color:'var(--muted)'}}/>
            <input placeholder="Name or phone" style={{height:36, padding:'0 10px 0 30px', border:'1px solid var(--border)', borderRadius:8, background:'white'}}/>
          </div>
        </div>
      </div>
      ...
      <div className="list-detail">
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {D.CUSTOMERS.map(c => (
                <tr key={c.id} className={sel.id===c.id?'selected':''} onClick={()=>setSel(c)} style={{cursor:'pointer'}}>
        ...
        <div className="card">
          <div className="card-head">
            <div style={{display:'flex', gap:10, alignItems:'center'}}>
              <div className="avatar" style={{width:40, height:40}}>{sel.name.split(' ').map(p=>p[0]).join('').slice(0,2)}</div>
              <div><h3>{sel.name}</h3><div className="muted" style={{fontSize:12}}>{sel.phone}</div></div>
            </div>
            <span className={"pill "+tierPill(sel.tier)}>{sel.tier}</span>
          </div>
          <div className="card-pad" style={{display:'grid', gap:12, fontSize:13}}>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
              <div><div className="muted" style={{fontSize:11}}>Loyalty points</div><div className="num strong" style={{fontSize:18}}>{sel.points.toLocaleString('en-IN')}</div></div>
              <div><div className="muted" style={{fontSize:11}}>Gift card balance</div><div className="num strong" style={{fontSize:18}}>{D.fmtINRRaw(sel.gift)}</div></div>
              <div><div className="muted" style={{fontSize:11}}>Lifetime spend</div><div className="num strong">{D.fmtINRRaw(sel.spend)}</div></div>
              <div><div className="muted" style={{fontSize:11}}>Avg. bill</div><div className="num strong">{D.fmtINRRaw(sel.avgBill)}</div></div>
              <div><div className="muted" style={{fontSize:11}}>Visits</div><div className="num strong">{sel.visits}</div></div>
              <div><div className="muted" style={{fontSize:11}}>Last visit</div><div className="strong">{sel.lastVisit}</div></div>
            </div>
          ...
          <div style={{display:'flex', gap:8, padding:14, borderTop:'1px solid var(--border-soft)'}}>
            ...
            <button className="btn btn-primary btn-sm" style={{flex:1}}><Icon.Mail size={12}/>Send offer</button>
          </div>
        </div>
      </div>
...
function Staff() {
  ...
  return (
    <Page3 title="Staff" sub="Roster, shifts, roles and permissions" actions={
      <>
        <button className="btn btn-ghost"><Icon.Calendar size={14}/>Schedule Shift</button>
        <button className="btn btn-primary"><Icon.Plus size={14}/>Add Staff</button>
      </>
    }>
      <div className="kpi-grid" style={{marginBottom:14}}>
      ...
      <div className="list-detail">
        ...
        <div className="card">
          ...
          <div style={{display:'flex', gap:8, padding:14, borderTop:'1px solid var(--border-soft)'}}>
            <button className="btn btn-ghost btn-sm" style={{flex:1}}><Icon.Edit size={12}/>Edit</button>
            <button className="btn btn-ghost btn-sm" style={{flex:1}}>Change role</button>
            <button className="btn btn-primary btn-sm" style={{flex:1}}>End shift</button>
          </div>
        </div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:14, marginTop:16}}>
        ...
        <div className="card card-pad">
          ...
          <button className="btn btn-soft btn-sm" style={{width:'100%', marginTop:10}}>Edit permission matrix</button>
        </div>
        <div className="card card-pad">
        ...
        <div className="card card-pad">
          ...
          <button className="btn btn-ghost btn-sm" style={{color:'var(--danger)', width:'100%'}}><Icon.Trash size={12}/>Off-board staff</button>
        </div>
      </div>
    </Page3>
  );
